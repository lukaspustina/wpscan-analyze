...
read -p "[INFO] Answer No to download binary from github and copy it to ${install_to} [y/n]" -r
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    if ! which cargo; then
        read -p "[QUESTION] Cargo is not detected. Do you want install Rust environment? [y/n]" -r
        echo    # (optional) move to a new line
        if [[ $REPLY =~ ^[Yy]$ ]]; then
